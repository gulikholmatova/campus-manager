    ...
    render() {
        ...
        return (
            <div>
                ...
                <button type="button" onClick={this.handleClick}>Update information</button>
                {this.state.active && <UpdateCampusForm />}
                {campus &&
                    <ul>
                        <li>Campus name: {name} </li>
                        <li>Campus image: <img src={imageUrl} /> </li>
                        <li>Campus address: {address} </li>
                        <li>Campus description: {description} </li>
                        <li>Students on this campus: {
                            students ?
                                <ol>
                                    {students.map((student) => {
                                        return (<li key={student.id}><Link to={`/students/${student.id}`}> {student.firstName} {student.lastName} </Link></li>)
                                    })}
                                </ol> :
