    ...
    render() {
        ...
        return (
            <div>
                ...
                <button type="button" onClick={this.handleClick}>Add a new student</button>
                {this.state.active && <NewStudentForm />}
                <table>
                    <tbody>
                        {students.map(student => (
                            <tr key={student.id}><td><button type='text' onClick={() => { this.props.deleteStudentFromServer(student.id) }}>
                                X
                            </button></td>
