    ...
    render() {
        ...
        return (
            <div>
                ...
                <button type="button" onClick={this.handleClick}>Add a new campus</button>
                {this.state.active && <NewCampusForm />}
                <table>
                    <tbody>
                        {campuses.map(campus => (
                            <tr key={campus.id}>
                                <td><button type='submit' onClick={() => { this.props.deleteCampusFromServer(campus.id) }}>
                                    X
                                </button></td>
                                <td><Link to={`campuses/${campus.id}`} key={campus.id}>
                                    {campus.name}
                                </Link></td>
